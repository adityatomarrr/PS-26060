...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#eef5f8] text-slate-800 flex">
      ...
      <aside className="sidebar-scroll w-64 border-r border-[#173246] bg-[#071521] p-5 hidden md:block sticky top-0 h-screen overflow-y-auto text-white">
...
function Overview({
  ...
  // Guard against a station with missing/malformed coordinates
  // crashing the whole map instead of just being skipped.
  const validStations = useMemo(
    () =>
      stations.filter(station => {
        ...
        if (!valid) {
          console.warn(
            `Skipping station "${station?.name || station?.id}" — invalid coordinates`
          );
        }
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-6">
        ...
        <div className="xl:col-span-2 border border-slate-200 rounded-2xl overflow-hidden shadow-[0_8px_30px_rgba(15,45,65,0.06)]">
          ...
          <div className="h-80 relative">
            ...
            {tileError && (
              <div className="absolute top-2 left-1/2 -translate-x-1/2 z-[1000] bg-amber-50 border border-amber-300 text-amber-800 text-xs px-3 py-1.5 rounded-full shadow">
              ...
              >
                ...
                {validStations.map(station => (
                  <React.Fragment key={station.id}>
                    {/* Larger invisible hit target — carries the click and popup, so you don't need to be pixel-precise on the small dot */}
                    <CircleMarker
                      center={[
                        station.location.latitude,
                        station.location.longitude
                    ...
                    {/* Small visible dot on top, purely decorative (clicks pass through to the hit target above) */}
                    <CircleMarker
                      center={[
                        station.location.latitude,
                        station.location.longitude
